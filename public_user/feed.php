...
$feedProgramPinsState = function_exists('home_feed_load_pins_state')
    ? home_feed_load_pins_state($dbh, $meId)
    : ['pins' => [], 'saved' => false];
$feedProgramPinSet = array_fill_keys((array)($feedProgramPinsState['pins'] ?? []), true);
...
if ($feedAlertPostId > 0 && $meId > 0) {
    try {
        ...
        $bootRow = $stBoot->fetch(PDO::FETCH_ASSOC) ?: null;
            if ($bootRow && publisher_can_view_post($dbh, $meId, $bootRow) && !post_is_slideshow_photos($bootRow)) {
            ...
            if (function_exists('msb_post_tags_people_for_post')) {
                $bootRow['tagged_people'] = msb_post_tags_people_for_post($dbh, (int)$bootRow['id']);
            } else {
                $bootRow['tagged_people'] = [];
            }
...
<html <?= app_html_lang_attrs(!empty($tabEmbed) ? 'tab-embed' : '') ?>>
  <head>
    ...
    <title><?= htmlspecialchars(app_t('Feed'), ENT_QUOTES, 'UTF-8') ?></title>
    <script>
      try{ if('scrollRestoration' in history) history.scrollRestoration = 'manual'; }catch(e){}
      (function(){
        var freshCreate = <?= !empty($feedFreshCreate) ? 'true' : 'false' ?>;
        if(freshCreate){
        ...
        var restartAtTop = <?= isset($_GET['top']) && (string)$_GET['top'] === '1' ? 'true' : 'false' ?>;
        try{
          restartAtTop = restartAtTop || sessionStorage.getItem('msbFeedStartAtTop') === '1';
          if(restartAtTop){
            sessionStorage.removeItem('msbFeedStartAtTop');
            sessionStorage.removeItem('msbResumePost');
            sessionStorage.removeItem('msbResumePostHome');
        ...
        if(!restartAtTop) return;

        function resetToFeedTop(){
          try{ window.scrollTo(0, 0); }catch(e){}
          var roots = [document.scrollingElement, document.documentElement, document.body];
          document.querySelectorAll('.sh-pagebody, .feed-desktop-center, .mf-feed, #mfFeed').forEach(function(el){
            roots.push(el);
          });
        ...
        document.addEventListener('DOMContentLoaded', function(){
          ...
          try{
            ...
            cleanUrl.searchParams.delete('top');
            cleanUrl.searchParams.delete('from_post');
          ...
          if(feed && typeof MutationObserver !== 'undefined'){
            var topObserver = new MutationObserver(function(){
              if(!feed.querySelector('.mf-card')) return;
              resetToFeedTop();
              window.requestAnimationFrame(resetToFeedTop);
              topObserver.disconnect();
            });
            topObserver.observe(feed, {childList:true, subtree:true});
          }
          window.requestAnimationFrame(function(){
            resetToFeedTop();
            window.requestAnimationFrame(resetToFeedTop);
          });
          window.setTimeout(resetToFeedTop, 300);
          window.setTimeout(resetToFeedTop, 900);
        }, {once:true});
        window.addEventListener('load', resetToFeedTop, {once:true});
      })();
    </script>
...
@media (max-width:767px){
  ...
  .mf-name.mf-name--sharing,
  .mf-name.is-sharing-with{
    white-space:normal;
    overflow:visible;
    text-overflow:unset;
    flex:1 1 auto;
  ...
  .mf-peer-link.is-sharing-with,
  .mf-meta:has(.msb-sharing-others-wrap){
    overflow:visible;
  }
  .mf-avatar-link{
    display:block;
  ...
  .mf-avatar-link:hover{opacity:.92;text-decoration:none;color:inherit;}
  a.msb-sharing-who{
    color:inherit;
    font-weight:700;
    text-decoration:none;
  ...
  .mf-head--on-media .msb-sharing-with,
  .mf-media-shell > .mf-head--on-media .msb-sharing-with{
    color:rgba(255,255,255,.72);
  }
  .mf-head--on-media .mf-name .mf-time,
  .mf-media-shell > .mf-head--on-media .mf-name .mf-time{
    color:rgba(255,255,255,.78);
  }
  .mf-head--on-media .mf-name .mf-dot,
  .mf-media-shell > .mf-head--on-media .mf-name .mf-dot{
  ...
  #pvOverlay .pv-name.is-sharing-with{
    white-space:normal;
    overflow:visible;
    text-overflow:unset;
    line-height:1.25;
  ...
  .mf-slide-summary .post-slide-summary-p{margin:0}
  .mf-slide-summary .post-slide-summary-list{margin:0;padding-left:1.15em;list-style:disc}
  .mf-slide-summary .post-slide-summary-list li{margin:0 0 .35em}
  .mf-slide-summary .post-slide-summary-list li:last-child{margin-bottom:0}
  ...
  .mf-act.is-love i{ color:var(--msb-rx-love, var(--msb-love-color, #ff4d6d)) !important; }
  .mf-act.is-like i{ color:var(--msb-rx-like, #2563eb) !important; }
...
@media (min-width:1025px){
  ...
  .mf-video-body{
    ...
    font-weight: 400;
    line-height: 1.45;
    color: var(--msb-palette-text, var(--feed-text, #344054));
    text-align:left;
  }
  ...
  .mf-card.is-single-video-post .media-stage.standard-video-stage > video{
    width:100%;
    height:auto;
    max-height:var(--post-media-max-height, min(56vh, calc(100dvh - 220px)));
    display:block;
    object-fit:contain;
    background:transparent;
  }
...
html[data-msb-appearance][data-theme="dark"] body.feed-page{
  ...
  --feed-text:var(--msb-palette-text);
  --feed-muted:var(--msb-palette-text-muted);
  --feed-soft-text:var(--msb-palette-text-muted);
  --feed-topbar-text:var(--msb-palette-text);
...
.pv-readmore,
a.msb-mention{
  color:var(--msb-palette-text, var(--feed-text, #0b1220));
  font-weight:800;
}
.pv-com .m .link{
  color:inherit;
...
.c-list .cmt .txt,
.pv-cap{
  color:var(--msb-palette-text, var(--feed-text));
}
.ig-media-caption{
  background:var(--feed-surface-strong);
  color:var(--msb-palette-text, var(--feed-text));
...
.jump-rail button{
  ...
  border:none;
  border-radius:50%;
  background:var(--msb-palette-surface-strong, #1c222b);
  color:var(--msb-palette-icon, #c5cdd6);
  box-shadow:0 10px 24px rgba(0,0,0,.18);
  display:flex;
  align-items:center;
  justify-content:center;
...
.jump-rail button:hover,
.jump-rail button:focus{
  outline:none;
  color:#fff;
}
@media (min-width:1025px){
...
@media (max-width:1024px){
  ...
  body.feed-page.feed-insta-ui .jump-rail button{
    width:40px;
    height:40px;
  }
}
        ...
        <div class="sh-mainpanel">
          ...
          <div class="sh-pagebody">
            ...
            <div class="feed-desktop-layout">
              <div class="feed-side-search" aria-label="<?= htmlspecialchars(app_t('Search feed'), ENT_QUOTES, 'UTF-8') ?>">
                <form id="feedTopSearchForm" class="feed-top-search-form feed-side-search-form" method="get" action="home.php">
                  ...
                  <div class="feed-top-search-field">
                    <button type="submit" class="feed-top-search-icon" aria-label="<?= htmlspecialchars(app_t('Search'), ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <input
                      ...
                      placeholder="<?= htmlspecialchars(app_t('Search'), ENT_QUOTES, 'UTF-8') ?>"
                      autocomplete="off"
                      enterkeyhint="search"
                    >
                  </div>
              ...
              <div class="feed-desktop-center">
                <div class="feed-top-search feed-top-search--tabs-only" aria-label="Explore feed">
                  <div class="feed-top-search-row feed-top-tabs-row">
                    <nav class="feed-discover-tabs" aria-label="<?= htmlspecialchars(app_t('Feed categories'), ENT_QUOTES, 'UTF-8') ?>">
                      <?php foreach ($feedDiscoverTabs as $tabKey => $tabLabel): ?>
                        <?php
                          ...
                          $optionalTabActive = $isOptionalDiscoverTab && ($feedDiscoverTab === $tabKey || isset($feedProgramPinSet[$tabKey]));
                    ...
                    <a class="feed-top-search-settings" href="settings.php" aria-label="Feed settings" title="Settings">
                ...
                (function(){
                  ...
                  function mfFeedEl(){ return document.getElementById('mfFeed'); }
                  function hostEl(){ return document.getElementById('homeDiscoverHost'); }
                  function frameEl(){ return document.getElementById('homeDiscoverFrame'); }
                  function jumpRailEl(){ return document.querySelector('.jump-rail'); }
                  function postedPillEl(){ return document.querySelector('.feed-desktop-center > .feed-posted-pill'); }
                  ...
                  function activateTabs(selected){
                  ...
                  function showCircle(){
                    var host = hostEl();
                    var mfFeed = mfFeedEl();
                    var jumpRail = jumpRailEl();
                    var postedPill = postedPillEl();
                    var search = document.querySelector('.feed-side-search');
                    if(host) host.hidden = true;
                    if(mfFeed) mfFeed.hidden = false;
                    if(jumpRail) jumpRail.hidden = false;
                    if(postedPill) postedPill.hidden = false;
                    if(search) search.hidden = false;
                  }
                  function showDiscover(link){
                    var mfFeed = mfFeedEl();
                    var host = hostEl();
                    var frame = frameEl();
                    var jumpRail = jumpRailEl();
                  ...
                  function switchTab(link){
                    ...
                    var selected = tabKeyFromLink(link);
                    try{ sessionStorage.setItem(storageKey, String(tabs.scrollLeft || 0)); }catch(err){}
                    activateTabs(selected);
                    if(isCircleTab(selected)) showCircle();
                    else showDiscover(link);
                    ...
                    if(searchForm){
                      ...
                      if(tabInput) tabInput.value = selected;
                    }
                    history.pushState({msbFeedTab:true}, '', link.href);
                    document.dispatchEvent(new CustomEvent('msb:feed-tab-change', {detail:{tab:selected}}));
                    if(window.MSBFeedPrograms && typeof window.MSBFeedPrograms.restore === 'function'){
                      window.MSBFeedPrograms.restore();
                    }
                  }
                  window.msbSwitchHomeTabLink = switchTab;
                  try{
                  ...
                  tabs.addEventListener('pointerover', function(e){
                    ...
                    var frame = frameEl();
                    if(!link || isCircleTab(tabKeyFromLink(link)) || !frame) return;
                    if(!frame.getAttribute('src')) frame.src = embedSrc(link);
                  });
            ...
            <?php
              ...
              $suggestedForYouMaxAdvertise = !empty($isPublisherWorkspaceViewer) ? 3 : 0;
              include __DIR__ . '/includes/suggested_for_you.php';
            ?>
            <div class="jump-rail" aria-label="Post navigation">
            ...
            (function(){
              function cards(){
                return Array.prototype.slice.call(document.querySelectorAll('#mfFeed .mf-card'));
              }
              function scrollRoot(){
                return document.querySelector('.feed-desktop-center > .mf-feed')
                  || document.getElementById('mfFeed')
                    ...
                    <div class="ig-media" id="pvMedia">
                      ...
                      <div class="ig-media-caption" id="pvCap" style="display:none;">
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-2">
                          ...
                          <a href="#" class="ig-cap-readmore" id="pvCapReadMore"><?php echo h(app_t('Read more')); ?></a>
                        </div>
                      </div>
                    </div>
        ...
        <div class="c-modal" role="dialog" aria-modal="true" aria-labelledby="cTitleText">
          ...
          <div class="c-topbar">
            <div class="c-left">
              ...
              <div class="c-headtxt">
                <div id="cTitleText" class="c-title"><?= htmlspecialchars(app_t('Comments'), ENT_QUOTES, 'UTF-8') ?></div>
                <div class="c-sub">
          ...
          <div class="c-footer">
            <form id="cCommentForm" class="m-0 w-100">
              ...
              <div class="input-group">
                <input type="text" id="cCommentText" class="form-control" placeholder="<?= htmlspecialchars(app_t('Write a comment...'), ENT_QUOTES, 'UTF-8') ?>">
...
/* ✅ CLAMPS */
        ...
        /* Meta pills inline after visibility icon in author name row */
        .mf-name-row{
          flex-wrap:wrap;
          row-gap:2px;
          align-items:baseline;
        }
        .mf-name-row.has-story{
          align-items:baseline;
        }
        .mf-meta{
          min-width:0;
        ...
        .mf-music-inline i{ color:#64748b; font-size:12px; }
        .mf-music-inline-text{
          min-width:0;
          overflow:hidden;
          text-overflow:ellipsis;
          white-space:nowrap;
        }
        .mf-meta-pills.is-story{
          display:inline;
          margin:0;
          padding:0;
          border:0;
        ...
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-story,
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-conn{
          color:rgba(255,255,255,.82);
        }
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-ent{
          color:#fff;
        }
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-sub-row,
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-music-inline,
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-sub-row .mf-time{
          color:rgba(255,255,255,.85);
        }
        body .mf-feed .mf-media-shell > .mf-head--on-media .mf-music-inline i{
          color:rgba(255,255,255,.85);
        }
        .mf-link-preview{
          display:block;
        ...
        .mf-link-preview:hover{ border-color:var(--msb-palette-action, rgba(37,99,235,.28)); box-shadow:0 10px 28px color-mix(in srgb, var(--msb-palette-action, #2563eb) 12%, transparent); }
        .mf-link-preview-main{
          display:flex;
          align-items:stretch;
          min-height:118px;
          height:180px;
        }
        .mf-link-preview.has-media .mf-link-preview-media{
          flex:0 0 36%;
          width:36%;
          max-width:220px;
        ...
        .mf-link-preview-media.is-logo,
        .mf-link-preview.has-logo .mf-link-preview-media{
          display:flex;
          align-items:center;
          justify-content:center;
          background:#fff;
        }
        .mf-link-preview-media img{
          width:100%;
          height:100%;
          object-fit:cover;
          display:block;
        ...
        .mf-link-preview-media.is-logo img,
        .mf-link-preview.has-logo .mf-link-preview-media img{
          width:44%;
          height:44%;
          max-width:88px;
          max-height:88px;
        ...
        .mf-link-preview-chips{
          display:flex;
          flex-wrap:wrap;
          gap:6px;
          margin-top:10px;
        }
        .mf-link-preview-chip{
          display:inline-flex;
          align-items:center;
          max-width:100%;
          padding:5px 10px;
          border-radius:999px;
          background:var(--msb-palette-hover-bg, #eef2f7);
        ...
        .mf-link-preview-cta{
          ...
          color:var(--msb-palette-text, var(--msb-palette-action, #1d4ed8));
          font-size:12.5px;
          font-weight:800;
          border-top:1px solid var(--msb-palette-border-strong, var(--msb-palette-border, rgba(37,99,235,.1)));
        }
        .mf-link-preview-cta .fa-chevron-right{ margin-left:auto; font-size:12px; }
        /* Discover-size lock: full column width + same card inset as public.php text posts */
        body.feed-insta-ui .mf-feed .mf-card:has(.mf-link-preview){
          width:100% !important;
          max-width:100% !important;
          margin-left:0 !important;
          margin-right:0 !important;
        ...
        body.feed-insta-ui .mf-feed .mf-card .mf-link-preview{
          width:100% !important;
          max-width:100% !important;
          box-sizing:border-box !important;
        }
        @media (max-width: 560px){
          .mf-link-preview-main{ flex-direction:column; min-height:0; height:auto; }
          .mf-link-preview.has-media .mf-link-preview-media{
            flex:0 0 auto;
            width:100%;
            max-width:none;
          ...
          .mf-link-preview-media img{ min-height:118px; height:118px; }
        }
...
@media (max-width:1024px){
  #btnOpenCommentsDrawer{
    ...
    <script>
      ...
      const API_URL = <?= json_encode('feed_api.php') ?>;
...
// Mirror rail clicks to existing controls (so existing logic stays unchanged)
        ...
        function isWithin24h(dt){
          try{
            ...
            if(m && typeof moment === 'function'){
              return moment().diff(m, 'hours', true) < 24;
            }
          }catch(e){}
          var raw = String(dt || '').trim();
          if(!raw) return false;
          var ms = Date.parse(raw.replace(' ', 'T'));
          if(!ms) ms = Date.parse(raw);
          if(!ms) return false;
        ...
        function isCircleFriendItem(it){
          var uid = Number(it && (it.user_id || it.author_id) || 0);
          if(uid > 0 && uid === Number(ME_ID || 0)) return false;
          var status = String((it && it.friend_status) || '').toLowerCase();
          if(status === 'friends') return true;
          var vis = String((it && it.visibility) || '').toLowerCase();
          return vis === 'friends';
        }
        function isCircleFeedCardItem(it){
          // Friend posts stay on Circle after 24h. Stranger / followed-publisher
          // cards still leave the main column (and the sidebar) after a day.
          if(isCircleFriendItem(it)) return true;
        ...
        function mfComputeDescMediaCardWidth(aspectW, aspectH, opts){
          return mfComputeMediaCardWidth(aspectW, aspectH, opts);
        }

        function mfComputeMediaCardWidth(aspectW, aspectH, opts){
          opts = opts || {};
          var availableWidth = mfFeedAvailableMediaWidth(opts);
        ...
        function mfInitialMediaCardStyleFromDims(dims, isPhoneShot){
          ...
          if(!safeWidth) return '';
          return '--post-media-card-width:'+mfMediaWidthCss(!!isPhoneShot, safeWidth)+';--post-media-max-height:'+mfMediaMaxHeightCss()+';width:100%;max-width:100%;margin-left:0;margin-right:0;padding:8px 12px;box-sizing:border-box;';
        }
        ...
        function mfApplyPublicVideoCardWidth(card, aspectW, aspectH){
          ...
          if(media){
            ...
            media.style.setProperty('align-items', 'flex-start', 'important');
            media.style.setProperty('justify-content', 'flex-start', 'important');
          ...
          if(video){
            ...
            video.style.setProperty('align-self', 'flex-start', 'important');
          ...
          if(image){
            ...
            image.style.setProperty('align-self', 'flex-start', 'important');
          ...
          var plate = media && media.querySelector(':scope > .msb-no-image');
          if(plate){
            plate.classList.remove('msb-phone-frame', 'msb-tablet-frame');
            ['width','min-width','max-width','height','min-height','max-height','aspect-ratio','margin','margin-top','margin-left','margin-right'].forEach(function(p){
              try{ plate.style.removeProperty(p); }catch(e){}
            });
            var actions = card.querySelector(':scope > .mf-actions');
            if(actions){
        ...
        function mfResyncAllStandardMediaCardWidths(){
          document.querySelectorAll('.mf-card.is-single-image-post, .mf-card.is-single-video-post').forEach(function(card){
            try{
              ...
              if(!dims || !dims.w || !dims.h){
                dims = {
                  w: Number(card.getAttribute('data-media-w') || card.getAttribute('data-device-w') || 0),
                  h: Number(card.getAttribute('data-media-h') || card.getAttribute('data-device-h') || 0)
                };
              }
        ...
        function updateViewerPills(hasMedia, attCount, dt){
          try{
            ...
            try{
              var $time = $('#pvTimeAgo');
              $time.siblings('.post-vis-badge').remove();
              if(window.MSBPostCardMenu && typeof window.MSBPostCardMenu.visibilityBadgeHtml === 'function' && window.currentPostVisibility){
                $time.after(' ' + window.MSBPostCardMenu.visibilityBadgeHtml('friends'));
              }
            }catch(eVis){}
            $('#pvMetaPills').hide();
          }catch(e){
        ...
        function openCommentsModal(){
          ...
          $('#cParentId').val('0');
          $('#cCommentText').val('');
          $('#cCommentText').attr('placeholder', (typeof msbT === 'function' ? msbT('Write a comment...') : 'Write a comment...'));
          $('#cCancelReply').hide();
        ...
        function renderCommentsModal(comments){
          ...
          function cmtHtml(x, depth){
            ...
            return ''
              ...
              + '          <div class="txt">'+(window.MSBMentionAC && window.MSBMentionAC.linkify ? window.MSBMentionAC.linkify(x.comment_text||'') : esc(x.comment_text||''))+'</div>'
              + '        </div>'
              + '        <div class="c-rowmeta">'
              + '          <span>'+esc(when)+'</span>'
        ...
        function mfBuildBodyHtml(className, text, maxSent){
          ...
          if(needsMore){
            return '<div class="'+esc(className)+' mf-body-has-more" data-full="'+esc(text)+'" data-expanded="0">'+
                     ...
                     '<a href="#" class="mf-readmore js-open-readmore-door">'+((typeof window.msbT==='function')?window.msbT('Read more'):'Read more')+'</a>'+
                   '</div>';
          }
        ...
        function bindMfStandardMediaCardSizing(scope){
          try{
            ...
            if(root.querySelectorAll){
              Array.prototype.forEach.call(root.querySelectorAll('.mf-card .msb-no-image'), function(ph){
                if(window.MSBNoImage && typeof window.MSBNoImage.markReady === 'function'){
                  window.MSBNoImage.markReady(ph);
                } else {
                  var card = ph.closest('.mf-card');
                  var stage = ph.closest('.media-stage, .mf-media');
                  if(stage) stage.classList.add('mf-media-sized');
                  if(card){
        ...
        function mfFeedHasPendingMedia($wrap){
          try{
            ...
            $wrap.find('.mf-card.is-single-video-post, .mf-card.is-single-image-post').each(function(){
              ...
              if(card.classList.contains('mf-video-error') || card.classList.contains('mf-image-error') || card.classList.contains('mf-media-missing')) return;
            ...
            if(mfFirstFeedCardIsReady($wrap) || (Date.now() - loadingStartedAt) >= 1500){
              ...
              try{
                if(FEED_FRESH_CREATE){
                  window.__MSB_SKIP_RESUME_HOME = true;
                  var pinCard = null;
                  var pinIdReveal = Number(FEED_PIN_POST_ID || 0);
                  if(pinIdReveal > 0){
                    pinCard = $wrap.find('.mf-card[data-id="'+String(pinIdReveal)+'"], .mf-card[data-post-id="'+String(pinIdReveal)+'"]').get(0);
                  }
                  if(!pinCard) pinCard = $wrap.children('.mf-card').get(0);
                  if(pinCard && typeof window.msbScrollFreshCreatePost === 'function'){
                    window.msbScrollFreshCreatePost(pinCard);
                    window.setTimeout(function(){ window.msbScrollFreshCreatePost(pinCard); }, 200);
                  } else {
                    try{ resetFeedCardPosition(); }catch(_rp){}
                  }
                } else if(window.MSBResumePost && typeof window.MSBResumePost.restoreHome === 'function'){
        ...
        function mfBuildHeadHtml(it, isOwner, pid, headerFollowHtml, onMedia){
          ...
          var nameHtml = '<a class="msb-sharing-who" href="'+esc(peerProfileHref(it, pid))+'" target="_top" rel="noopener">'+esc(name || '')+'</a>';
          var storyHtml = mfMetaStoryHtml(it);
          var musicHtml = mfMetaMusicInlineHtml(it);
          var visHtml = (window.MSBPostCardMenu && typeof window.MSBPostCardMenu.visibilityBadgeHtml === 'function')
            ? window.MSBPostCardMenu.visibilityBadgeHtml('friends')
            : '';
          var hasStory = !!storyHtml;
          return ''+
            '<div class="'+headClass+'">'+
              '<div class="mf-peer-link">'+
                '<a class="mf-avatar-link" href="'+esc(peerProfileHref(it, pid))+'" target="_top" rel="noopener" aria-label="Open '+esc(name||'User')+' profile">'+
        ...
        function mfLocationPrep(location){
        ...
        function mfMetaStoryHtml(it){
          ...
          if (location) {
            var prep = mfLocationPrep(location);
        ...
        function mfMetaMusicInlineHtml(it){
        ...
        function mfMetaPillsHtml(it){
          // Legacy name — story sentence UI.
          return mfMetaStoryHtml(it);
        }

        function mfFirstLinkMeta(it){
          ...
          if (!url) return { url: '', host: '', title: '', description: '', image: '' };
          var host = '';
          try {
            host = String(new URL(url).hostname || '').replace(/^www\./i, '');
          } catch (eHost) { host = ''; }
          function isJunkPreview(s){
            s = String(s || '').toLowerCase();
            return /request has been blocked|access denied|just a moment|forbidden|captcha|unusual traffic/.test(s);
          }
          function hostTitle(h){
            h = String(h || '');
            var base = h.replace(/\.(com|net|org|io|co|app|dev|ai|edu|gov|uk|us|ca|au|de|fr|info|biz)(\.[a-z]{2})?$/i, '');
            base = base.replace(/\..+$/, '') || h;
            base = base.replace(/[-_]/g, ' ').trim();
            if (!base) return h || 'Visit link';
            return base.replace(/\b\w/g, function(c){ return c.toUpperCase(); });
          }
          var title = String((it && it.link_title) || '').trim();
          if (isJunkPreview(title)) title = '';
          if (!title) title = String((it && it.title) || '').trim();
          if (!title || isJunkPreview(title)) title = hostTitle(host);
          var description = String((it && it.link_description) || '').trim();
          if (isJunkPreview(description)) description = '';
          if (!description) description = 'Open this website to see the full page, details, and latest updates.';
          var image = String((it && it.link_image) || '').trim();
          var imageFallback = '';
          var isLogo = false;
          if (host) {
            imageFallback = 'https://www.google.com/s2/favicons?sz=256&domain_url=' + encodeURIComponent('https://' + host);
            if (!image) {
              image = 'https://unavatar.io/' + encodeURIComponent(host) + '?fallback=false';
              isLogo = true;
            } else if (/unavatar\.io|google\.com\/s2\/favicons/i.test(image)) {
              isLogo = true;
            }
          }
          var tags = [];
          var rawTags = String((it && it.link_tags) || '').trim();
          if (rawTags) {
            rawTags.split(/[,|;]+/).forEach(function(t){
              t = String(t || '').replace(/\s+/g, ' ').trim();
              if (!t || t.length > 32 || isJunkPreview(t)) return;
        ...
        function mfMusicRowHtml(it){
          // Kept for compatibility; cards now use mfMetaPillsHtml (includes music).
          return '';
        }
        ...
        function mfMountFollowOnMedia($card, it){
          ...
          if(!$media.length){
            var $menu = $card.find('.mf-head .mf-menu-wrap, .mf-head .post-card-menu-wrap').first();
            if($menu.length){
        ...
        function mfNoImageHtml(){
          ...
          return '<div class="msb-no-image msb-media-unavailable" role="img" aria-label="Media unavailable">'+
        ...
        function mfSetCarouselIndex($carousel, nextIndex){
          ...
          // Presentation: keep super title + intro fixed; sync subtitle/summary only.
          try {
        ...
        function mfRenderCardShell(it){
          ...
          /* Text/link posts match Discover full-column width (phone-shot is media-only). */
          var isPhoneShot = !!deviceMeta.phone_shot && !!psrc;
          var isTabletShot = !!deviceMeta.tablet_shot && !isPhoneShot && !!psrc;
          ...
          var mediaW = Number(it.preview_w || 0);
          var mediaH = Number(it.preview_h || 0);
          if(previewMissing && deviceDims && deviceDims.w && deviceDims.h){
            mediaW = deviceDims.w;
            mediaH = deviceDims.h;
          } else if((!mediaW || !mediaH) && deviceDims && deviceDims.w && deviceDims.h){
            mediaW = deviceDims.w;
            mediaH = deviceDims.h;
          }
          if(!mediaW || !mediaH){
            if(shapeClass === 'single-landscape'){ mediaW = 16; mediaH = 9; }
            else if(shapeClass === 'single-square'){ mediaW = 1; mediaH = 1; }
            else if(shapeClass === 'single-portrait' || isPhoneShot){ mediaW = 9; mediaH = 16; }
            else { mediaW = 4; mediaH = 3; }
          }
          if(previewMissing && !shapeClass && isSingleMedia){
            if(mediaH > mediaW * 1.1) shapeClass = 'single-portrait';
            else if(mediaW > mediaH * 1.15) shapeClass = 'single-landscape';
            else shapeClass = 'single-square';
          }
          if(previewMissing && (!deviceStyle || deviceStyle.indexOf('--device-ar-w') < 0)){
            mediaStyleAttr = ' style="--device-ar-w:'+esc(String(mediaW))+';--device-ar-h:'+esc(String(mediaH))+';"';
          }
          deviceDataAttrs += ' data-media-w="'+esc(String(mediaW))+'" data-media-h="'+esc(String(mediaH))+'"';
              ...
              mediaHtml = '<div class="'+imageMediaClass+'"'+mediaStyleAttr+' data-shape-ready="1" data-count="'+attCount+'" data-index="0">'+
                          (isMultiMedia
                            ? ('<div class="media-carousel mf-media-carousel" data-index="0" data-pending-hydrate="1">'+
                               ...
                               '</div>')
                            : (previewInner || ('<img src="'+esc(psrc)+'" alt="" loading="eager" decoding="sync" fetchpriority="high">')))+
                          '</div>';
            } else if(pkind === 'video'){
              if(isReelCard){
                ...
                mediaHtml = ''+
                  '<div class="mf-media">'+
                    (previewInner || ('<video class="ig-smart-feed-video js-mf-reel-video" src="'+esc(psrc)+'" playsinline muted loop preload="none" data-smart-video="1"></video>'))+
                    mfMediaDots(attCount)+
                  '</div>';
                ...
                var videoPosterAttr = (!previewMissing && pthumb) ? (' poster="'+esc(pthumb)+'"') : '';
                mediaHtml = ''+
                  '<div class="'+videoMediaClass+'"'+mediaStyleAttr+' data-shape-ready="'+shapeReady+'">'+
                    (previewInner || ('<video class="ig-smart-feed-video" src="'+esc(psrc)+'"'+videoPosterAttr+' playsinline muted preload="auto" data-smart-video="1"></video>'))+
                    mfMediaDots(attCount)+
                  '</div>';
                actionsHtml = normalActions();
              }
            } else {
              mediaHtml = previewMissing ? mfNoImageHtml() : mfFileTileHtml(psrc, pkind);
            }
          }
          ...
          if(!isLiveCard && !isReelCard && !isMultiMedia && isSingleMedia && hasMedia){
            if(previewMissing){
              initialCardStyle = mfInitialMediaCardStyleFromDims(
                { w: mediaW, h: mediaH },
                isPhoneShot
              );
            } else if(pkind === 'video'){
        ...
        function mfSyncCardFriendMenu($card, status){
          ...
          if(window.MSBPostCardMenu && typeof window.MSBPostCardMenu.syncCardFriend === 'function'){
            window.MSBPostCardMenu.syncCardFriend($card, status);
          } else {
            $card.find('.mf-menu .mf-unfriend, .post-card-menu .pcm-unfriend').each(function(){
          ...
          $('.mf-card[data-peer-id="'+peerId+'"]').each(function(){
            ...
            if(status === 'friends'){
              ...
              $card.find('.mf-media-top-actions').each(function(){
                if(!this.querySelector('.mf-friend-btn, .publisher-follow-btn, .mf-publisher-follow')){
                  this.remove();
                }
              });
              return;
            }
            // After Unfriend (or pending→none), restore the + button before fries.
            if(!$card.find('.mf-friend-btn:not(.mf-publisher-follow)').length){
              mfRemountFollowOnCard($card, {
                user_id: peerId,
              ...
              $card.find('.mf-friend-btn:not(.mf-publisher-follow)').each(function(){
                var btn = this;
                btn.disabled = false;
                btn.setAttribute('data-status', status || 'none');
                btn.classList.remove('is-friends', 'is-pending', 'is-accept');
                btn.classList.add('primary');
                if(btn.classList.contains('mf-media-action-circle')){
        ...
        window.mfSyncFriendUiForPeer = mfSyncFriendUiForPeer;
        ...
        function isSlideshowPhotoPost(it){
          if(!it || isStoryPost(it)) return false;
        ...
        function isFeedCardPost(it){
          return !isStoryPost(it) && !isSlideshowPhotoPost(it);
        }
        ...
        function applySearchFilter(){
          ...
          // Main Circle cards keep friends' posts after 24h. Sidebar rows stay 24h-only.
          items = items.filter(function(it){
            return isCircleFeedCardItem(it);
          });
          ...
          try{
            if(FEED_FRESH_CREATE || window.__MSB_SKIP_RESUME_HOME){
              window.__MSB_SKIP_RESUME_HOME = true;
            } else if(window.MSBResumePost && typeof window.MSBResumePost.restoreHome === 'function'){
              window.MSBResumePost.restoreHome();
            }
        ...
        function mergeFeedItemsPreserve(incoming, previous, pinId){
          ...
          // If a freshly painted card already had tags, don't lose them when the list
          // row briefly arrives without tagged_people.
          if(previous.length){
            var prevById = {};
            previous.forEach(function(p){
              var pid = Number(p && p.id || 0);
              if(pid > 0) prevById[pid] = p;
            });
            incoming = incoming.map(function(it){
              var id = Number(it && it.id || 0);
              if(id <= 0 || !prevById[id]) return it;
              var prevTags = Array.isArray(prevById[id].tagged_people) ? prevById[id].tagged_people : [];
              var nextTags = Array.isArray(it.tagged_people) ? it.tagged_people : [];
              if(!nextTags.length && prevTags.length){
                it = $.extend({}, it, { tagged_people: prevTags });
              }
              return it;
            });
          }
        ...
        function upsertFeedItem(row){
          ...
          if(idx >= 0){
            var prev = allItems[idx] || {};
            var merged = $.extend({}, prev, row);
            var prevTags = Array.isArray(prev.tagged_people) ? prev.tagged_people : [];
            var nextTags = Array.isArray(row.tagged_people) ? row.tagged_people : [];
            merged.tagged_people = nextTags.length ? nextTags : prevTags;
            allItems[idx] = merged;
          } else {
            allItems.unshift(row);
          }
            ...
            function finishRefresh(){
              ...
              if(pinId > 0){
                ...
                if(FEED_FRESH_CREATE || window.__MSB_SKIP_RESUME_HOME){
                  try{
                    window.__MSB_SKIP_RESUME_HOME = true;
                    var freshCard = document.querySelector(
                      '#mfFeed .mf-card[data-id="' + String(pinId) + '"],' +
                      '#mfFeed .mf-card[data-post-id="' + String(pinId) + '"]'
                    );
                    if(freshCard && typeof window.msbScrollFreshCreatePost === 'function'){
            ...
            $.getJSON(API_URL, { ajax:'view', id:postId, lite:1, _: Date.now() }, function(viewRes){
              ...
              if(row && Number(row.id || 0) === postId){
                try{
                  ...
                  try{
                    window.__MSB_SKIP_RESUME_HOME = true;
                    var softCard = document.querySelector(
                      '#mfFeed .mf-card[data-id="' + String(postId) + '"],' +
                      '#mfFeed .mf-card[data-post-id="' + String(postId) + '"]'
                    );
                    if(softCard){
                      softCard.classList.add('is-alert-focus');
                      if(typeof window.msbScrollFreshCreatePost === 'function'){
                        window.msbScrollFreshCreatePost(softCard);
                        window.setTimeout(function(){ window.msbScrollFreshCreatePost(softCard); }, 280);
                      }
                    }
                  }catch(_soft){}
                  return;
        ...
        function setViewerHeader(authorText, dt, post){
          ...
          window.currentPostVisibility = (window.MSBPostCardMenu && window.MSBPostCardMenu.normalizeVisibility)
            ? window.MSBPostCardMenu.normalizeVisibility((post && post.visibility) || 'friends')
            : String((post && post.visibility) || 'friends').toLowerCase();
          ...
          $('#pvTimeAgo').text(timeAgoShort(dt) || ' ').siblings('.post-vis-badge').remove();
          if(window.MSBPostCardMenu && typeof window.MSBPostCardMenu.visibilityBadgeHtml === 'function'){
            $('#pvTimeAgo').after(' ' + window.MSBPostCardMenu.visibilityBadgeHtml('friends'));
          }
        ...
        function renderMainMedia(src, kind){
          ...
          var currentAtt = (window.__feedMediaAtts || [])[Number(currentMediaIdx || 0)] || {};
          if(mfAttIsMissing(currentAtt) && (kind === 'image' || kind === 'gif' || kind === 'video')){
            $('#pvMedia').html(mfNoImageHtml());
            return;
          }
...
<style>
  ...
  .pv-left{flex:1.15;min-width:0;background:var(--msb-palette-bg, #0b1220);display:flex;align-items:center;justify-content:center;}
  ...
  .pv-right{flex:.85;min-width:320px;display:flex;flex-direction:column;background:#fff;min-height:0;border-left:1px solid var(--msb-palette-border, rgba(15,23,42,.18));}
...
@media (min-width:1025px){
  ...
  body .mf-feed .mf-name-row > .mf-meta-pills{
    margin:0 !important;
    flex:1 1 auto !important;
  }
  ...
  body .mf-feed .mf-media-shell > .mf-head--on-media .mf-meta-pills{
    margin-top:0;
  }
  body .mf-feed .mf-media-shell > .mf-head--on-media .mf-meta-pill{
  ...
  body .mf-feed .mf-media-shell > .mf-head--on-media .mf-meta-pill i{
    color:#fff;
    opacity:.95;
  }
  ...
  body .mf-feed .mf-video-body{
    ...
    color:var(--msb-palette-text, var(--feed-text, #eef4ff)) !important;
  ...
  body .mf-feed .mf-body p,
  body .mf-feed .mf-video-body .mf-body-formatted,
  body .mf-feed .mf-video-body .post-card-paragraph,
  body .mf-feed .mf-video-body p{
...
// Prefer this opener (same scope as #pvOverlay) so mention/tag hideNav works.
...
window.MSBClosePostViewer = function(postId){
  ...
  try {
    if (typeof window.MSBFeedRemoveDeletedPost === 'function' && postId > 0) {
      window.MSBFeedRemoveDeletedPost(postId);
    }
  } catch (eRm) {}
};
function pvRenderCaption(post, atts, slideIndex){
  ...
  const superTitleRaw = (post?.title || '').toString().trim();
  const introRaw = (post?.body || post?.description || '').toString().trim();
  const taggedForCap = Array.isArray(post?.tagged_people) ? post.tagged_people : [];
  const superTitle = (window.MSBPostCardMenu && typeof window.MSBPostCardMenu.displayTextWithoutTagHandles === 'function')
    ? window.MSBPostCardMenu.displayTextWithoutTagHandles(superTitleRaw, taggedForCap)
    : superTitleRaw;
  const intro = (window.MSBPostCardMenu && typeof window.MSBPostCardMenu.displayTextWithoutTagHandles === 'function')
  ...
  // Nothing to show
  if (!title && !desc && !slideTitle && !slideDesc) {
    pv.caption.style.display = 'none';
    pv.caption.innerHTML = '';
    return;
  }
  ...
  if (desc) {
    ...
    if (!t.truncated) {
      introHtml = `<div class="pv-cap-desc pv-cap-intro">${pvFormatRichText(t.full)}</div>`;
    } else {
      introHtml = `
        <div class="pv-cap-desc pv-cap-intro">
  ...
  const subHtml = (anySlideText && slideTitle) ? `<div class="pv-cap-subtitle">${pvEsc(slideTitle)}</div>` : '';
...
function pvSyncMediaOrientation(){
  const ov = (window.pv && pv.ov) ? pv.ov : document.getElementById('pvOverlay');
  if (!ov || !window.pv || !pv.media) return;
  let el = null;
...
function pvApplyCounts(data){
  ...
  if (post.created_at) {
    ...
    if (window.MSBPostCardMenu && typeof window.MSBPostCardMenu.visibilityBadgeHtml === 'function') {
      pv.meta.insertAdjacentHTML('beforeend', ' ' + window.MSBPostCardMenu.visibilityBadgeHtml('friends'));
    }
  }
...
async function pvLoad(postId){
  postId = Number(postId || 0);
  if (typeof window.__pvLoadSeq !== 'number') window.__pvLoadSeq = 0;
  const seq = ++window.__pvLoadSeq;
  ...
  if (pv.ov) pv.ov.classList.toggle('pv-is-switching', alreadyOpen && hadContent);
  // Keep current media/caption/comments while fetching — avoids bright Loading flash on next/prev.
  if (!(pv.media && pv.media.childElementCount)) {
    pv.media.innerHTML = `<div class="pv-loading-only" style="display:flex;align-items:center;justify-content:center;min-width:min(360px,42vw);height:100%;background:var(--msb-palette-bg, #0b1220);color:var(--msb-palette-text-muted, rgba(255,255,255,.7));">Loading…</div>`;
  }
  try {
    ...
    if (seq !== window.__pvLoadSeq) return;
    ...
    if (seq === window.__pvLoadSeq && pv.ov) pv.ov.classList.remove('pv-is-switching');
  }
}
...
pv.next.addEventListener('click', () => { if (window.__pvHidePostNav) return; if (pvIndex < pvIds().length - 1) pvOpenByIndex(pvIndex + 1); });
...
(function(){
  ...
  var alertHideNav = <?php echo ((int)($_GET['hide_nav'] ?? 0) === 1) ? 'true' : 'false'; ?>;
  var alertFreshCreate = <?php echo !empty($feedFreshCreate) ? 'true' : 'false'; ?>;
  ...
  function feedScrollRoot(){
    var mf = document.getElementById('mfFeed')
      || document.querySelector('.feed-desktop-center > .mf-feed');
    if (mf && !mf.hidden) {
      try{
        var cs = window.getComputedStyle(mf);
        if (cs.display !== 'none' && cs.visibility !== 'hidden') return mf;
      }catch(_cs){ return mf; }
    }
  ...
  function stickyOverlayPad(root){
    var pad = 10;
    if (!root) return pad;
    var rootTop = 0;
      ...
      [
        ...
        try{
          doc.querySelectorAll(sel).forEach(function(el){
            ...
            // Only count chrome that actually covers the scrollport top.
            if (bottom > rootTop + 1) overlap.push(bottom - rootTop);
          });
        }catch(_q){}
      });
    }
    try{ collect(document, window); }catch(_d){}
    try{
      if (window.parent && window.parent !== window) {
        collect(window.parent.document, window.parent);
      }
    }catch(_p){}
  ...
  function scrollCardShowTop(card){
    if (!card) return;
    var root = feedScrollRoot();
    var offset = stickyOverlayPad(root);
    var behavior = alertFreshCreate ? 'auto' : 'smooth';
    try{
      if (root && root.contains && root.contains(card)) {
        var rootRect = root.getBoundingClientRect();
        var cardRect = card.getBoundingClientRect();
        // Align card top under overlay chrome so name / friends / story stay visible.
  ...
  try{ window.msbScrollFreshCreatePost = scrollCardShowTop; }catch(_ex){}
  ...
  function highlightAlertCard(card){
    ...
    if (alertFreshCreate) {
      try{ window.__MSB_SKIP_RESUME_HOME = true; }catch(_sk){}
      scrollCardShowTop(card);
      // Re-assert after list refresh / media paint so the header stays in view.
      [120, 280, 600, 1100, 1800].forEach(function(ms){
        window.setTimeout(function(){ scrollCardShowTop(card); }, ms);
      });
      return;
    }
    try{
  ...
  function runAlertOpen(){
    try{
      // After create-post, stay on Circle list — do not open the post/comments modal.
      if (alertFreshCreate && !(alertCommentId > 0)) {
        var attemptsFresh = 0;
        (function waitFreshFeedCard(){
          attemptsFresh += 1;
          ...
          if (attemptsFresh < 40) window.setTimeout(waitFreshFeedCard, 160);
      ...
      if (typeof window.pvOpenById === 'function') {
        ...
        if (alertCommentId > 0) {
          ...
          (function waitComment(){
            triesC += 1;
            try {
              if (typeof window.pvFocusCommentById === 'function' && window.pvFocusCommentById(alertCommentId)) {
                clearAlertParams();
                return;
              }
            } catch (eFocus) {}
...
html:not([data-theme="dark"]):not(.dark-auto) body.feed-insta-ui .mf-act:not(.is-love):not(.is-like):not(.is-reacted),
html:not([data-theme="dark"]):not(.dark-auto) body.feed-insta-ui .mf-act:not(.is-love):not(.is-like):not(.is-reacted) i,
html:not([data-theme="dark"]):not(.dark-auto) body.feed-insta-ui .mf-act:not(.is-love):not(.is-like):not(.is-reacted) .mf-num,
...
html:not([data-theme="dark"]):not(.dark-auto) body.feed-insta-ui .has-rx-icon[data-selected-reaction="like"] .msb-pact-thumb{
  color:var(--msb-rx-like, #2563eb)!important;
...
html:not([data-theme="dark"]):not(.dark-auto) body.feed-insta-ui .has-rx-icon[data-selected-reaction="love"] .msb-pact-heart{
  color:var(--msb-rx-love, #ff4d6d)!important;
  -webkit-text-fill-color:var(--msb-rx-love, #ff4d6d)!important;
}
html:not([data-theme="dark"]):not(.dark-auto) body.feed-insta-ui .has-rx-icon[data-selected-reaction="dislike"] .msb-pact-thumb-down{
...
/* Match Discover: keep the ellipsis and Read more on the final preview line. */
body.feed-insta-ui .mf-body-has-more > .mf-body-formatted.is-clamped{
  display:contents !important;
  max-height:none !important;
  overflow:visible !important;
}
body.feed-insta-ui .mf-body-has-more > .mf-body-formatted.is-clamped .post-card-paragraph:last-child{
  display:inline !important;
  margin-bottom:0 !important;
}
body.feed-insta-ui .mf-body-has-more > .mf-body-formatted.is-clamped + .mf-readmore{
  display:inline !important;
...
body.feed-insta-ui .mf-feed > .mf-card.mf-card-media-head-outside > .mf-slide-title,
body.feed-insta-ui .mf-feed > .mf-card.mf-card-media-head-outside > .mf-slide-summary{
  padding-left:0 !important;
  padding-right:0 !important;
  margin-left:0 !important;
  margin-right:0 !important;
...
body.feed-insta-ui .mf-feed > .mf-card.mf-card-media-head-outside > .mf-body .post-slide-summary-list{
  margin-left:0 !important;
  padding-left:0 !important;
  text-indent:0 !important;
}
...
<style id="feed-description-left-lock">
/* Keep post description on the same left edge as avatar / action icons. */
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-body,
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-video-body,
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-title,
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-slide-title,
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-slide-summary,
body.feed-insta-ui .mf-feed .mf-card > .mf-body,
body.feed-insta-ui .mf-feed .mf-card > .mf-video-body,
body.feed-insta-ui .mf-feed .mf-card > .mf-title,
body.feed-insta-ui .mf-feed .mf-card > .mf-slide-title,
body.feed-insta-ui .mf-feed .mf-card > .mf-slide-summary{
  padding-left:0 !important;
  padding-right:0 !important;
  margin-left:0 !important;
  margin-right:0 !important;
...
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-body .mf-body-formatted,
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-body .post-card-paragraph,
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-body p,
body.feed-insta-ui .mf-feed .mf-card > .mf-body .mf-body-formatted,
body.feed-insta-ui .mf-feed .mf-card > .mf-body .post-card-paragraph,
body.feed-insta-ui .mf-feed .mf-card > .mf-body p{
  margin-left:0 !important;
  padding-left:0 !important;
  text-indent:0 !important;
  text-align:left !important;
}
body.feed-page.feed-insta-ui .mf-feed .mf-card > .mf-slide-summary .post-slide-summary-list,
body.feed-insta-ui .mf-feed .mf-card > .mf-slide-summary .post-slide-summary-list{
...
@media (max-width:767.98px){
  ...
  body.feed-insta-ui .mf-feed .mf-card.mf-card-description-media{
    --desc-media-max:100%;
    --post-media-max-height:min(46vh, calc(100dvh - 270px));
  }
}
...
(function(){
  ...
  function leftAlignCard(card){
    ...
    if(!maxH || /px$/i.test(maxH)){
      ...
      if(window.matchMedia('(max-width: 767.98px)').matches) maxH = 'min(42vh, calc(100dvh - 300px))';
      else if(window.matchMedia('(max-width: 1024.98px)').matches) maxH = 'min(52vh, calc(100dvh - 240px))';
      else maxH = 'min(56vh, calc(100dvh - 220px))';
    }
    ...
    if (card.classList.contains('mf-card-description-media') && mediaEl) {
      ...
      if (!descMaxH) {
        descMaxH = (typeof mfMediaMaxHeightCss === 'function') ? mfMediaMaxHeightCss() : '78vh';
      }
...
<style id="circle-post-card-spacing-css">
/* Circle only: visually separate each post without changing its controls or media logic. */
...
body.feed-page.feed-insta-ui #mfFeed.mf-feed > .mf-card{
  margin:0 0 12px !important;
  border:1px solid var(--msb-palette-border, var(--feed-border-strong, rgba(148,163,184,.28))) !important;
  border-radius:8px !important;
  background:var(--msb-palette-surface, var(--feed-surface, #fff)) !important;
  box-shadow:0 2px 8px rgba(15,23,42,.06) !important;
  overflow:visible !important;
}
body.feed-page.feed-insta-ui #mfFeed.mf-feed > .mf-card:last-child{
  margin-bottom:0 !important;
}
body.feed-page.feed-insta-ui #mfFeed.mf-feed > .mf-card::after{
  content:none !important;
  display:none !important;
}
@media (max-width:575.98px){
