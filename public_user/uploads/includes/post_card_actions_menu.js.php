(function(window){
  ...
  function viewerOwnsMenuTarget(el){
    var wrap = el && el.closest ? el.closest('.post-card-menu-wrap, #ttStoriesMenuWrap, .tt-stories-menu-wrap, .pcm-menu-portal') : null;
    ...
    if(wrap && String(wrap.getAttribute('data-is-owner') || '') === '1' && peerId <= 0) return false;
    return false;
  }

  function stripOwnerOnlyMenuItems(menu){
    if(!menu || !menu.querySelectorAll) return;
    menu.querySelectorAll('.pcm-edit, .pcm-delete, .pcm-archive, .pcm-private').forEach(function(el){
  ...
  function buildItems(it, isOwner, pid, helpers){
    ...
    if(!feedSurface && !isPublisher && peerId && canShowAddFriend(friendStatus, isOwner) && !staffReadonly){
