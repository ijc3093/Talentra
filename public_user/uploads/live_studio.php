...
$prefillVisibility = strtolower(trim((string)($currentLive['visibility'] ?? 'friends')));
if (!in_array($prefillVisibility, ['private', 'friends', 'public'], true)) {
    $prefillVisibility = 'friends';
}
...
$doorPanelMode = isset($_GET['door_panel']) && (string)$_GET['door_panel'] === '1';
$hubEmbedMode = isset($_GET['hub_embed']) && (string)$_GET['hub_embed'] === '1';
$rightDoorMode = isset($_GET['right_door']) && (string)$_GET['right_door'] === '1';
$initialStudioSource = strtolower(trim((string)($_GET['source'] ?? 'webcam')));
if (!in_array($initialStudioSource, ['webcam', 'software'], true)) {
    $initialStudioSource = 'webcam';
}
if ($rightDoorMode) {
    $initialStudioSource = 'software';
}
$studioBodyClasses = array_filter([
    $doorPanelMode ? 'door-panel-mode' : '',
    $hubEmbedMode ? 'hub-embed-mode' : '',
    $rightDoorMode ? 'right-door-mode' : '',
]);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .studio-intro-stack { display: none !important; }
    .history-panel { display: none !important; }
    .stats-chart { display: none !important; }
    .studio-frame > .panel:first-child { display: none !important; }
    body { padding-bottom: 0 !important; }
    body.door-panel-mode {
      background: var(--bg);
    }
    body.door-panel-mode .studio-shell {
      min-height: 100%;
      padding-left: 0;
      overflow: visible;
    }
    body.door-panel-mode .studio-main {
      padding: 0;
    }
    body.door-panel-mode .studio-frame {
      width: 100%;
      height: auto;
      min-height: 100%;
      border: 0;
    ...
    body.door-panel-mode .panel {
      border-right: 0;
      border-bottom: 1px solid #dfe5f1;
      overflow: visible;
    }
    body.door-panel-mode .panel:last-child {
      border-bottom: 0;
    }
    body.door-panel-mode .panel-form .chat-card {
      display: none !important;
    }
    body.door-panel-mode .toggle-card .action-row {
      grid-template-columns: repeat(3, minmax(0, 1fr));
      gap: 6px;
    }
    body.door-panel-mode .toggle-card .action-row #endLiveButton {
      grid-column: 1 / -1;
    }
    body.door-panel-mode .toggle-card .action-row .primary-btn,
    body.door-panel-mode .toggle-card .action-row .accent-btn,
    body.door-panel-mode .toggle-card .action-row .secondary-btn {
      min-width: 0;
      width: 100%;
      padding: 7px 6px;
      font-size: 10px;
    ...
    body.hub-embed-mode.door-panel-mode,
    body.hub-embed-mode.door-panel-mode .studio-shell,
    body.hub-embed-mode.door-panel-mode .studio-main {
      background: var(--msb-palette-bg, #ffffff);
    }
    body.hub-embed-mode.door-panel-mode .studio-frame {
      background: var(--msb-palette-bg, #ffffff);
    }
    body.hub-embed-mode textarea,
    body.door-panel-mode textarea {
      resize: none;
    }
    body.right-door-mode .source-grid {
      grid-template-columns: 1fr;
    }
    body .sh-logopanel,
    body .sh-headpanel {
    ...
    .camera-preview .preview-camera-controls .toggle-state {
      color: rgba(255,255,255,.92);
      flex: 0 0 auto;
      white-space: nowrap;
    }

    .camera-preview .preview-camera-controls .cam-btn {
      min-width: 0;
      padding: 7px 10px;
      font-size: 11px;
      white-space: nowrap;
    ...
    .preview-camera-controls.toggle-top {
      flex-wrap: nowrap;
    }
    ...
    .toggle-card .action-row #endLiveButton {
      grid-column: 1 / -1;
    }

    .toggle-card .action-row .primary-btn,
    .toggle-card .action-row .accent-btn,
    .toggle-card .action-row .secondary-btn {
      min-width: 0;
      width: 100%;
      padding: 8px 10px;
      font-size: 11px;
    ...
    @media (max-width: 1560px) {
      .studio-frame {
        grid-template-columns: minmax(500px, 1fr) minmax(390px, 1fr);
      }
    }
...
<body<?php echo $studioBodyClasses ? ' class="' . h(implode(' ', $studioBodyClasses)) . '"' : ''; ?>>
  ...
  <div class="studio-shell">
    <aside class="studio-rail" aria-label="Studio navigation">
      <a class="brand-mark" href="feed.php" aria-label="Talsora">t</a>
    ...
    <main class="studio-main">
      <div class="studio-frame">
        <section class="panel">
          ...
          <div class="studio-intro-stack" aria-hidden="true">
        ...
        <section class="panel panel-video">
            <div class="source-grid" id="sourceGrid">
              <?php if (!$rightDoorMode): ?>
              <button class="source-card<?= $initialStudioSource === 'webcam' ? ' is-selected' : '' ?>" type="button" data-source="webcam">
                <span class="source-glyph"><?php echo iconWebcam(); ?></span>
                <span class="source-name">Webcam</span>
              </button>
              <?php endif; ?>
              <button class="source-card<?= $initialStudioSource === 'software' ? ' is-selected' : '' ?>" type="button" data-source="software">
          ...
          <div class="camera-preview" id="cameraPreview">
            ...
            <div class="preview-toolbar">
              <div class="preview-camera-controls toggle-top">
          ...
          <form class="form-grid" action="#" method="post" onsubmit="return false;">
            ...
            <div class="split">
              <div class="field">
                ...
                <div class="select-wrap">
                  ...
                  <p class="studio-field-note" id="sessionVisibilityNote" style="margin-top:8px;font-size:13px;color:var(--studio-muted,#667085);"></p>
            ...
            <div class="toggle-card">
              ...
              <div class="action-row">
                ...
                <?php if ($doorPanelMode): ?>
                <button class="secondary-btn" type="button" id="studioBackButton">Back</button>
                <?php endif; ?>
  ...
  <script>
    ...
    const isDoorPanelMode = <?php echo $doorPanelMode ? 'true' : 'false'; ?>;
    const isHubEmbedMode = <?php echo $hubEmbedMode ? 'true' : 'false'; ?>;
    const isRightDoorMode = <?php echo $rightDoorMode ? 'true' : 'false'; ?>;
    ...
    const state = {
      ...
      source: <?php echo json_encode($initialStudioSource, JSON_UNESCAPED_SLASHES); ?>,
      softwareMode: 'file',
      softwareFile: null,
      softwareFileName: '',
      softwarePageUrl: '',
    ...
    function syncVisibilityNote() {
      const note = document.getElementById('sessionVisibilityNote');
      if (!note || !visibilityInput) return;
      const value = String(visibilityInput.value || 'friends').toLowerCase();
      if (value === 'private') {
        note.textContent = 'Private rooms are only visible to you in Chat.';
      } else if (value === 'friends') {
        note.textContent = 'Friends-only rooms appear in the Friend tab on Friends Feed.';
      } else {
        note.textContent = 'Public rooms appear in the Public tab on Public and on Public Live.';
      }
    }

    function postHubLiveRefresh() {
      const refreshPayload = { type: 'msb-hub-live-refresh' };
      function postTo(target) {
        if (!target) return;
        try { target.postMessage(refreshPayload, '*'); } catch (error) {}
      }
    ...
    function syncSummary() {
      ...
      previewStatusPill && (previewStatusPill.innerHTML = `<span class="tiny-dot"></span>${statusLabel(status)}`);
    ...
    function renderRoomData(payload) {
      ...
      if (isHubEmbedMode && payload) {
        try {
          window.parent.postMessage({ type: 'msb-hub-room-data', payload: payload }, '*');
        } catch (error) {}
      }
    }
    ...
    function stopCameraStream() {
      ...
      stopSnapshotLoop();
      stopRtcPolling();
      publishHubHostStreamToParent(0);
    }
    ...
    function buildLiveRightDoorUrl(extraQuery){
      ...
      var bases = [];
      try {
        if (window.top && window.top.location) bases.push(window.top.location.href);
      } catch (error) {}
      try {
        if (window.parent && window.parent !== window && window.parent.location) {
          bases.push(window.parent.location.href);
        }
      } catch (error) {}
      bases.push(window.location.href);
      for (var i = 0; i < bases.length; i += 1) {
        try {
          return new URL(path, bases[i]).href;
        } catch (error) {}
      }
      return path;
    }

    function isInsideRightLiveDoor(){
      try {
        ...
        return hubWindow.frameElement.id === 'ttLiveRightDoorFrame';
      } catch (error) {
        return false;
      }
    }
    ...
    function openLiveRightDoor(extraQuery){
      var url = buildLiveRightDoorUrl(extraQuery);
      var hosts = [];
      try {
      ...
      if (window.parent && window.parent !== window) {
        try {
          (window.top || window.parent).postMessage({ type: 'msb-live-right-door-open', url: url }, '*');
          return true;
        } catch (error) {}
      }
    ...
    function publishHubHostStreamToParent(liveId) {
      if (!isHubEmbedMode) return;
      try {
        ...
        if (isLive && id > 0 && state.mediaStream) {
          ...
          window.parent.__msbHubHostStream = null;
          window.parent.__msbHubHostStreamLiveId = 0;
        }
      } catch (error) {}
    }
    ...
    function openLiveDoorHub() {
      ...
      try {
        window.parent.postMessage({ type: 'msb-live-door-open', url: hubUrl }, '*');
      } catch (error) {}
    ...
    async function runStudioAction(action, options = {}) {
      ...
      try {
        ...
        syncSnapshotLoop();
        syncRtcHostLoop();
        if (action === 'save_draft' && state.live && String(state.live.status || '').toLowerCase() === 'live') {
          postHubLiveRefresh();
        }
        if (action === 'start_live' && state.live && Number(state.live.id || 0) > 0) {
          ...
          if (isDoorPanelMode && isHubEmbedMode) {
            ...
            try {
              ...
              for (var depth = 0; depth < 3 && ancestor; depth += 1) {
                try {
                  if (ancestor.parent && ancestor.parent !== ancestor) {
                    ancestor = ancestor.parent;
                    postHubLiveMessages(ancestor);
                  } else {
                    break;
                  }
                } catch (relayError) {
              ...
              try {
                if (window.top && window.top !== window) {
                  postHubLiveMessages(window.top);
                }
              } catch (relayError) {}
            } catch (error) {}
            return true;
    ...
    sourceCards.forEach((card) => {
      card.addEventListener('click', () => {
        ...
        if (nextSource === 'software') {
          if (isInsideRightLiveDoor()) {
            applySoftwareSourceLocally();
            return;
          }
          if (openLiveRightDoor()) {
            return;
          }
        }
    ...
    visibilityInput?.addEventListener('change', syncVisibilityNote);
    syncVisibilityNote();
    ...
    if (studioBackButton) {
      studioBackButton.addEventListener('click', openLiveDoorHub);
    }
    ...
    if (autoOpenLiveWatchId > 0 && !(isDoorPanelMode && isHubEmbedMode)) {
      window.setTimeout(function() {
    ...
    window.addEventListener('message', function(event) {
      if (!isHubEmbedMode || !event || !event.data || typeof event.data !== 'object') return;
      if (event.data.type !== 'msb-hub-device-control' && event.data.type !== 'msb-hub-request-room-data') return;
      if (event.data.type === 'msb-hub-request-room-data') {
        fetchJsonSafe('ajax/live_studio_room_action.php', {
          method: 'GET',
          credentials: 'same-origin',
          cache: 'no-store'
        }).then(function(data) {
          ...
          try {
            window.parent.postMessage({ type: 'msb-hub-room-data', payload: data }, '*');
          } catch (error) {}
          renderRoomData(data);
        }).catch(function() {});
        return;
      }
      if (event.data.device === 'mic') {
        state.micOn = !!event.data.enabled;
        syncStudioLiveDeviceControls();
        return;
      }
      if (event.data.device === 'camera') {
        state.deviceCameraEnabled = !!event.data.enabled;
        syncStudioLiveDeviceControls();
        persistStudioCameraEnabled();
      }
    });
