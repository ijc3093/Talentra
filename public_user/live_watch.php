...
if (!$live) {
    ...
    $canView = live_browse_can_view($dbh, $meId, $live);
    if (!$canView) {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body.door-panel-mode .watch-shell,
    body.door-panel-mode .watch-frame {
      min-height: 100vh;
      height: 100vh;
      background: #090b10;
    }
    ...
    body.door-panel-mode .watch-grid,
    body.door-panel-mode .watch-frame.has-chat .watch-grid {
      display: grid;
      grid-template-columns: 1fr;
      grid-template-rows: minmax(34vh, 42vh) minmax(0, 1fr);
      min-height: 0;
    }
    body.door-panel-mode .watch-sidebar,
    body.door-panel-mode .watch-frame.has-chat .watch-sidebar {
      display: grid !important;
      border-left: 0;
      border-top: 1px solid rgba(255,255,255,.08);
      min-height: 0;
    }
    body.door-panel-mode .watch-stage,
    body.door-panel-mode .stage-screen {
      min-height: 34vh;
      height: 100%;
    }
    body.door-panel-mode .watch-bottom {
      display: none;
    }
    body.door-panel-mode .watch-side-stats {
      display: none;
    }
    body.door-panel-mode .watch-head {
      padding-top: 10px;
      padding-bottom: 8px;
    }
    body.door-panel-mode .watch-top-end {
      min-width: 34px;
      width: 34px;
      height: 34px;
      padding: 0;
    ...
    body.door-panel-mode .watch-top-end::before {
      content: '×';
      font-size: 24px;
      line-height: 1;
      font-weight: 400;
    ...
    body.hub-panel-mode,
    body.hub-panel-mode .watch-shell {
      min-height: 100%;
      height: 100%;
      background: #11131a;
    }
    body.hub-panel-mode .watch-frame {
      width: 100%;
      min-height: 100%;
      height: 100%;
      max-width: none;
    ...
    body.hub-panel-mode .watch-grid,
    body.hub-panel-mode .watch-frame.has-chat .watch-grid {
      min-height: 0;
    }
    body.hub-panel-mode .watch-top-end {
      min-width: 34px;
      width: 34px;
      height: 34px;
      padding: 0;
    ...
    body.hub-panel-mode .watch-top-end::before {
      content: '×';
      font-size: 24px;
      line-height: 1;
      font-weight: 400;
    ...
    body.hub-embed-mode .watch-head,
    body.hub-embed-mode .watch-sidebar,
    body.hub-embed-mode .watch-bottom,
    body.hub-embed-mode .watch-side-stats,
    body.hub-embed-mode .stage-center,
    body.hub-embed-mode .stage-note,
    body.hub-embed-mode .stage-top {
      display: none !important;
    }
    ...
    body.hub-embed-mode .hub-owner-cam-tap.is-visible {
      display: flex;
    }
    body.hub-embed-mode .hub-watch-connecting,
    body.embed-mode.hub-stage-mode .hub-watch-connecting {
      position: absolute;
      inset: 0;
      z-index: 6;
      display: none;
    ...
    html.hub-stage-mode,
    html.hub-stage-mode body,
    body.embed-mode.hub-stage-mode .watch-shell,
    body.embed-mode.hub-stage-mode .watch-frame,
    body.embed-mode.hub-stage-mode .watch-grid,
    body.embed-mode.hub-stage-mode .watch-stage,
    body.embed-mode.hub-stage-mode .stage-screen {
      width: 100%;
      height: 100%;
      min-height: 100%;
      max-height: 100%;
    ...
    body.embed-mode.hub-stage-mode .stage-image,
    body.embed-mode.hub-stage-mode .stage-video,
    body.embed-mode.hub-stage-mode .stage-screen video {
      width: 100%;
      height: 100%;
      object-fit: cover;
      background: #000;
...
<body class="<?php echo trim(($embedMode ? 'embed-mode ' : '') . ($headerModalMode ? 'header-modal-mode ' : '') . ($doorPanelMode ? 'door-panel-mode ' : '') . ($hubEmbedMode ? 'hub-embed-mode ' : '') . ($hubPanelMode ? 'hub-panel-mode ' : '') . ($hubStageMode ? 'hub-stage-mode ' : '')); ?>">
      ...
      <?php else: ?>
        <div class="watch-head">
          ...
          <div class="watch-head-right">
            ...
            <button type="button" class="watch-top-end" id="watchEndButtonTop" aria-label="Close live"><?php echo ($doorPanelMode || $hubEmbedMode || $hubPanelMode) ? '×' : 'Leave'; ?></button>
          </div>
        </div>
        ...
        <div class="watch-grid">
          <section class="watch-stage">
            <div class="stage-screen">
              ...
              <?php if ($hubEmbedMode || $hubStageMode || !$embedMode): ?>
              <div class="watch-stage-reactions" id="watchStageReactions" aria-hidden="true"></div>
              <div class="hub-watch-connecting" id="hubWatchConnecting" aria-live="polite">Connecting to live...</div>
              <?php endif; ?>
  ...
  <script>
    ...
    const isHubStageMode = <?php echo $hubStageMode ? 'true' : 'false'; ?>;
    const isHubPanelMode = <?php echo $hubPanelMode ? 'true' : 'false'; ?>;
    const preferSnapshotViewer = <?php echo $preferSnapshotInHub ? 'true' : 'false'; ?>;
    let peerKeyNonce = 0;
    let peerKey = '';
    ...
    let ownerStreamBorrowedFromParent = false;
    ...
    function syncHubWatchConnectingUi() {
    ...
    function notifyHubWatchVideoState(isReady) {
      if ((!isHubEmbedMode && !(isHubStageMode && isEmbedMode)) || isOwnerView) {
        return;
      }
      try {
        window.parent.postMessage({
          type: isReady ? 'msb-hub-watch-video-ready' : 'msb-hub-watch-video-lost',
          liveId: liveId
        }, '*');
      } catch (error) {}
    }
    ...
    function markWebRtcReady(isReady) {
      ...
      if (isReady && !isOwnerView && !viewerVideoHasFrames()) {
        if (!pendingWebRtcReadyTimer) {
          pendingWebRtcReadyTimer = window.setTimeout(function() {
            pendingWebRtcReadyTimer = null;
      ...
      if (pendingWebRtcReadyTimer) {
        clearTimeout(pendingWebRtcReadyTimer);
        pendingWebRtcReadyTimer = null;
      }
    ...
    async function startOwnerLocalPreview(forceDirectCapture) {
      ...
      if (isHubEmbedMode && !forceDirectCapture) {
        for (let attempt = 0; attempt < 12; attempt += 1) {
          const parentStream = getParentHubHostStream();
          if (parentStream && await attachOwnerLocalStream(parentStream, true)) {
            return;
          }
      ...
      try {
        ...
        await attachOwnerLocalStream(stream, false);
      } catch (error) {
        webRtcReady = false;
        if (isHubEmbedMode) {
          syncSnapshotPolling();
    ...
    function getParentHubHostStream() {
      if (!isHubEmbedMode) {
        return null;
      }
      try {
        ...
        if (!parentStream || parentLiveId !== liveId) {
          return null;
        }
        const videoTracks = typeof parentStream.getVideoTracks === 'function' ? parentStream.getVideoTracks() : [];
    ...
    async function attachOwnerLocalStream(stream, borrowedFromParent) {
      if (!stream || !watchStageVideo) {
        syncHubOwnerCamTap();
        return false;
      }
      ownerStreamBorrowedFromParent = !!borrowedFromParent;
      localOwnerStream = stream;
      watchStageVideo.muted = true;
      watchStageVideo.defaultMuted = true;
      watchStageVideo.setAttribute('muted', 'muted');
      watchStageVideo.srcObject = stream;
      ...
      syncSnapshotPolling();
      if (isHubEmbedMode) {
        syncOwnerHubSnapshotLoop();
      }
      syncHubOwnerCamTap();
      return true;
    }

    function syncHubOwnerCamTap() {
      if (!hubOwnerCamTap || !isHubEmbedMode || !isOwnerView) {
        return;
      }
      const needsTap = isLiveActive && !localOwnerStream && !webRtcReady;
      hubOwnerCamTap.classList.toggle('is-visible', needsTap);
      hubOwnerCamTap.setAttribute('aria-hidden', needsTap ? 'false' : 'true');
    }

    async function uploadOwnerSnapshotFrame() {
      if (ownerSnapshotBusy) return;
      if (!isOwnerView || !isHubEmbedMode || !isLiveActive || !localOwnerStream || !watchStageVideo) {
        return;
      }
      if (!watchCameraEnabled || watchStageScreen.classList.contains('is-local-video-off')) {
        return;
      }
      if (watchStageVideo.readyState < 2 || !watchStageVideo.videoWidth || !watchStageVideo.videoHeight) {
        return;
      }
      ...
      try {
        ...
        // keep owner snapshot upload silent
      } finally {
        ownerSnapshotBusy = false;
      }
    }

    function stopOwnerHubSnapshotLoop() {
      if (ownerSnapshotTimer) {
        clearInterval(ownerSnapshotTimer);
        ownerSnapshotTimer = null;
      }
      ownerSnapshotBusy = false;
    }

    function syncOwnerHubSnapshotLoop() {
      if (!isOwnerView || !isHubEmbedMode || !isLiveActive || !localOwnerStream) {
        stopOwnerHubSnapshotLoop();
        return;
      }
      if (ownerSnapshotTimer) {
        return;
    ...
    if (watchEndButton) {
      watchEndButton.addEventListener('click', function() {
        leaveLivePresence();
        if (isHubPanelMode) {
          try { window.parent.postMessage({ type: 'msb-hub-watch-close' }, '*'); } catch (e) {}
          return;
        }
    ...
    if (watchEndButtonTop) {
      watchEndButtonTop.addEventListener('click', function() {
        if (isHubEmbedMode || isHubPanelMode) {
          try { window.parent.postMessage({ type: 'msb-hub-watch-close' }, '*'); } catch (e) {}
          return;
        }
        if (isDoorPanelMode) {
          try {
            if (window.parent && window.parent.TTLive && typeof window.parent.TTLive.close === 'function') {
              window.parent.TTLive.close();
              return;
            }
          } catch (e) {}
          try { window.parent.postMessage({ type: 'msb-live-door-close' }, '*'); } catch (e2) {}
          return;
        }
        if (watchEndButton) {
    ...
    if (isDoorPanelMode && watchFrame) {
      watchFrame.classList.add('has-chat', 'sidebar-mode-chat');
      setWatchSidebarMode('chat');
    }

    if (isHubPanelMode && watchFrame) {
      watchFrame.classList.add('has-chat', 'sidebar-mode-chat');
      setWatchSidebarMode('chat');
    }
    ...
    syncHubOwnerCamTap();
    if (hubOwnerCamTap) {
      hubOwnerCamTap.addEventListener('click', function() {
        startOwnerLocalPreview(true);
      });
    }
    window.addEventListener('beforeunload', function() {
      ...
      if (ownerSnapshotTimer) clearInterval(ownerSnapshotTimer);
      ...
      if (localOwnerStream && !ownerStreamBorrowedFromParent) {
        localOwnerStream.getTracks().forEach(function(track) {
          track.stop();
        });
      }
