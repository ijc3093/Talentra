...
(function () {
  ...
  function openCreatePostModal(src){
    if (!createPostModal || !createPostModalFrame) return;
  ...
  function closeCreatePostModal(){
    if (!createPostModal || !createPostModalFrame) return;
    createPostModal.classList.remove('is-open');
    createPostModal.setAttribute('aria-hidden', 'true');
    createPostModalFrame.setAttribute('src', 'about:blank');
  ...
  pollUnreadThreads();
  pollNotificationCount();
  setInterval(() => { pollUnreadCount(); pollUnreadThreads(); pollNotificationCount(); }, 4000);
