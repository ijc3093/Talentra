...
{
    try {
        $st = $dbh->prepare('SHOW COLUMNS FROM `' . str_replace('`', '', $table) . '` LIKE :col');
...
function publisher_org_fetch_public_user_orgs(PDO $dbh, int $publisherUserId): array
{
    ...
    try {
        ...
        $orgId = (int)($org['id'] ?? 0);
        if ($orgId > 0 && publisher_org_db_column_exists($dbh, 'organizations', 'publisher_user_id')) {
            $stLink = $dbh->prepare('
                UPDATE organizations
                SET publisher_user_id = :uid, updated_at = NOW()
                WHERE id = :id
                  AND (publisher_user_id IS NULL OR publisher_user_id = 0 OR publisher_user_id = :uid)
            ');
            $stLink->execute([':uid' => $publisherUserId, ':id' => $orgId]);
        }
        ...
        if (publisher_org_db_column_exists($dbh, 'organizations', 'publisher_user_id')) {
            $st = $dbh->prepare('
                ...
                WHERE status = 1
                  ...
                  AND (publisher_user_id IS NULL OR publisher_user_id = 0 OR publisher_user_id = :uid)
            ');
            $st->execute([':uid' => $publisherUserId, ':name' => $publisherName]);
        }
