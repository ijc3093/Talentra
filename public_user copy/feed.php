...
@media (max-width:767px){
  ...
  body .mf-feed .mf-card:has(.pcm-wrap-open),
  body .mf-feed .mf-card:has(.post-card-menu.open){
  ...
  /* --- Mobile/Tablet 3-dots menu (per post card) --- */
  ...
  .mf-menu-btn:active{ background:rgba(0,0,0,.06); }
  .mf-menu{
    position:absolute;
    top:38px;
    right:0;
  ...
  .mf-menu a:hover, .mf-menu button:hover{ background:rgba(8,97,188,.08); }
  .mf-menu .mf-del{ color:#b42318; }
  .mf-menu .mf-del:hover{ background:rgba(180,35,24,.10); }
  .mf-menu .mf-unfriend{ color:#b42318; }
  .mf-menu .mf-unfriend:hover{ background:rgba(180,35,24,.10); }
  .mf-menu .mf-publisher-unfollow{ color:#b42318; }
  .mf-menu .mf-publisher-unfollow:hover{ background:rgba(180,35,24,.10); }
  ...
  .mf-act.is-love i{ color:#ea445a !important; }
...
<style><?php include __DIR__ . '/includes/feed_page_chrome.css.php'; ?></style>
  ...
  <?php $feedLeftRailActive = 'feed.php'; include __DIR__.'/includes/feed_left_rail.php'; ?>
    ...
    <div id="lbOverlay" class="lb-overlay" style="display:none;"></div>
        <div class="sh-mainpanel">
          ...
          <div class="sh-pagebody">
            <div class="ig-feed-header">
              ...
              <div class="ig-stories-wrap">
                <div class="ig-stories-bar is-empty" aria-label="Stories">
                  <div class="ig-stories-track is-empty has-create" id="igStoriesTrack">
              ...
              <div class="ig-feed-top-actions" aria-label="Header actions">
                ...
                <button type="button" class="ig-top-act ig-top-live" aria-label="Go live"><i class="fa fa-video-camera"></i><span>Live</span></button>
            ...
            <?php $feedRightRailActive = 'feed.php'; include __DIR__ . '/includes/feed_right_rail.php'; ?>
                        ...
                        <div class="ig-underbar ig-insta-actionbar">
                          <div class="ig-insta-actions-left ig-actions ig-actions-icons">
                            ...
                            <a id="btnShare" class="ig-act" type="button" title="Share">
                              <span class="ig-circle"><i class="fa fa-paper-plane-o ig-insta-ico-share"></i></span>
                            </a>
      ...
      <style>
        ...
        */
        ...
        .ig-viewer-body.mode-image-overlay .ig-image-overlay-btn.is-love .io-ico i{ color:#ec4899; }
...
/* ✅ CLAMPS */
        ...
        .ig-post-footer-act.rx-love i{ color:#ec4899 !important; }
        ...
        .ig-viewer-body.mode-reel .ig-vrail .vrail-btn.rx-love .vrail-ico i{ color:#ec4899 !important; }
    ...
    <script>
      ...
      (function(){
        ...
        function openCommentsTray(){
          ...
          // ✅ If no post selected yet, use active/first post from list
          if(!pid){
            try{
          ...
          // Prefer the leftbar "Comments" tray (with X-circle close) when available
          if(window.TTComments && typeof window.TTComments.setPost === 'function'){
            // If we already have comments for this post, use them immediately
            ...
            // Otherwise fetch fresh comments (safe fallback)
            $.getJSON(API_URL, { ajax:'comments', id:pid, post_id:pid }, function(res){
              ...
              // If API fails, fall back to the center modal
...
// Mirror rail clicks to existing controls (so existing logic stays unchanged)
        ...
        $(document).on('click', function(e){
          ...
          if(target.closest('#tt-menu-wrap, #tt-comments-wrap, #tt-readmore-wrap, #tt-profile-wrap, #tt-stories-wrap, #ttMenuClose, #ttCommentsClose, #ttRmClose, #ttProfileClose, #ttStoriesClose')) return;
          if(target.closest('.ig-stories-menu-btn, .ig-story-item, .js-open-profile-door, .js-open-messages-door, .js-open-notifications-door, .js-open-friend-requests-door, .feed-ig-avatar, .mf-comment, #commentCountLink, #commentCountLinkV, #btnViewComments, #btnFooterComment, #btnFooterViewComments, .ig-image-overlay-btn[data-act="comment"], #pvCapReadMore, .ig-cap-readmore, #pvFooterReadMore, #pvInlineReadMore, #btnReadMore')) return;
        ...
        function openReadMore(){
          // ✅ NEW: open Read More in LEFTBAR (no modal)
        ...
        // ✅ Read more link opens LEFTBAR drawer (no modal)
        $(document).on('click', '#pvCapReadMore, .ig-cap-readmore', function(e){
          e.preventDefault();
          var opened = false;
          try{ opened = !!openReadMore(); }catch(err){}
          if(opened) try{ openLeftbar(); }catch(_e){}
        });
        $(document).on('click', '#pvFooterReadMore', function(e){
          e.preventDefault();
          var opened = false;
          try{ opened = !!openReadMore(); }catch(err){}
          if(opened) try{ openLeftbar(); }catch(_e){}
        ...
        // ✅ Inline "Read more" (Rule #3: under media, at end of long description)
        $(document).on('click', '#pvInlineReadMore', function(e){
          e.preventDefault();
          var opened = false;
          try{ opened = !!openReadMore(); }catch(err){}
          if(opened) try{ openLeftbar(); }catch(_e){}
        ...
        if(window.TTReadMore){
          try{
            var originalFeedRmClose = window.TTReadMore.close;
            window.TTReadMore.close = function(){
              try{ closeLeftbar(); }catch(e){ try{ document.body.classList.remove('lb-open'); }catch(_e){} }
              if(typeof originalFeedRmClose === 'function') originalFeedRmClose();
            };
          }catch(e){}
        }
        ...
        function mfBuildBodyHtml(className, text, maxSent){
          ...
          if(sc >= maxSent){
            return '<div class="'+esc(className)+' mf-body-has-more" data-full="'+esc(text)+'" data-expanded="0">'+
                     ...
                     '<a href="#" class="mf-readmore">Read more</a>'+
                   '</div>';
          }
        ...
        function mfBuildMenuWrapHtml(it, isOwner, pid, onMedia){
          onMedia = !!onMedia;
          var iconClass = onMedia ? 'fa fa-ellipsis-h' : 'fa fa-ellipsis-v';
          return ''+
        ...
        function mfRenderCardShell(it){
          ...
          if(!isLiveCard && !hasMedia && !title && !hasBody) return '';
          ...
          if(isLiveCard){
            ...
            mediaHtml = ''
              ...
              +               '<a class="mf-live-action-btn mf-act mf-love" type="button" title="Love"><i class="fa fa-heart-o"></i><span class="mf-num mf-love">0</span></a>'
              ...
              +               '<a class="mf-live-action-btn mf-act mf-comment" type="button" title="Comment"><i class="fa fa-comment-o"></i><span class="mf-num mf-cmt">0</span></a>'
              +               '<a class="mf-live-action-btn mf-act mf-share" type="button" title="Share"><i class="fa fa-share-square-o"></i><span class="mf-num mf-share">0</span></a>'
              +               '<span class="mf-live-action-spacer"><a class="mf-live-action-btn mf-act mf-save" type="button" title="Save"><i class="fa fa-bookmark-o"></i><span class="mf-num mf-save">0</span></a></span>'
              +             '</div>'
        ...
        function mfHydrateCard(postId, post, counts, atts){
          ...
          $card.find('.mf-act').removeClass('is-love is-like');
          ...
          if(Number(c.is_saved||0) === 1) $card.find('.mf-act.mf-save').addClass('is-save');
          if(Number(c.is_shared||0) === 1) $card.find('.mf-act.mf-share').addClass('is-share');
        ...
        function mfOpenReadMoreDrawer($card, bodyText){
          ...
          if(window.TTReadMore && typeof window.TTReadMore.toggle === 'function'){
            ...
            if(opened){
              try{ openLeftbar(); }catch(e){ try{ document.body.classList.add('lb-open'); }catch(_e){} }
            }
            return;
          }
          try{
            ...
            try{ openLeftbar(); }catch(_e){}
            openReadMore();
          }catch(e){}
        }

        // Event delegation for mobile cards
        $(document).on('click', '.mf-reel-body .mf-readmore', function(e){
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
          var $body = $(this).closest('.mf-reel-body');
          var $card = $(this).closest('.mf-card');
          if(!$body.length || !$card.length) return;
          mfOpenReadMoreDrawer($card, String($body.attr('data-full') || ''));
        });

        $(document).on('click', '.mf-body .mf-readmore, .mf-reel-body .mf-readmore, .mf-video-body .mf-readmore', function(e){
          if($(this).closest('.mf-reel-body').length) return;
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
          var $b = $(this).closest('.mf-body, .mf-video-body');
          var $card = $(this).closest('.mf-card');
          if(!$b.length || !$card.length) return;
          mfOpenReadMoreDrawer($card, String($b.attr('data-full') || ''));
        });
        ...
        function setStoriesBarEmptyState(isEmpty){
          ...
          track.classList.add('has-create');
        }
...
<style>
  ...
  .pv-act.is-love{color:#ef2b7b;} /* pink/red */
...
@media (min-width:1025px){
  ...
  body .mf-feed .mf-menu.open:not(.pcm-menu-portal){
    display:block !important;
  }
  ...
  body .mf-feed .mf-menu a,
  body .mf-feed .mf-menu button{
    color:var(--feed-text) !important;
    font-size:13px !important;
    font-weight:600 !important;
  }
...
<script>
// ✅ Open the same Post Viewer Modal when clicking the feed post card
(function(){
  function curPostId(){
    var el = document.getElementById('curPostId');
    if(!el) return 0;
    return Number(el.value || el.getAttribute('value') || 0);
  }
  // Open by postId (works even if list ids not ready)
  ...
  function clickedPostId(target){
    try{
      var row = target && target.closest ? target.closest('.pl-item[data-id], .mf-card[data-id], [data-post-id]') : null;
      if (row){
        var rid = Number(row.getAttribute('data-id') || row.getAttribute('data-post-id') || 0);
        if (rid) return rid;
      }
      var active = document.querySelector('#postList .pl-item.active[data-id]');
      if (active){
        var aid = Number(active.getAttribute('data-id') || 0);
        if (aid) return aid;
      }
    }catch(err){}
    return curPostId();
  ...
  document.addEventListener('click', function(e){
    var card = e.target.closest && e.target.closest('.ig-post-card, .mf-card');
    if(!card) return;
    // Don't hijack clicks on controls/inputs/menus
    if (e.target.closest('.post-card-menu-wrap, .post-card-menu, .mf-menu.post-card-menu, .post-card-menu-btn, a, button, input, textarea, select, label, .dropdown, .dropdown-menu, .pl-dots, .comment-actions, .mf-actions, .mf-top')) return;
    var pid = clickedPostId(e.target);
    if(pid) window.pvOpenById(pid);
  }, true);
})();
</script>
